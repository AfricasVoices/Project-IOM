...
echo "Copying $INPUT_PIPELINE_CONFIGURATION -> $container_short_hash:/data/pipeline-configuration.json"
docker cp "$INPUT_PIPELINE_CONFIGURATION" "$container:/data/pipeline-configuration.json"

mkdir -p "$OUTPUT_RAW_DATA_DIR"
echo "Copying $OUTPUT_RAW_DATA_DIR/. -> $container_short_hash:/data/Raw Data/"
docker cp "$OUTPUT_RAW_DATA_DIR/." "$container:/data/Raw Data/"

# Run the container
echo "Starting container $container_short_hash"
docker start -a -i "$container"

# Copy the output data back out of the container
echo "Copying $container_short_hash:/data/Raw Data/. -> $OUTPUT_RAW_DATA_DIR"
docker cp "$container:/data/Raw Data/." "$OUTPUT_RAW_DATA_DIR"

if [[ "$PROFILE_CPU" = true ]]; then
    echo "Copying $container_short_hash:/data/cpu.prof -> $CPU_PROFILE_OUTPUT_PATH"
